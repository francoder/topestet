...
<form id="form-filter-data" class="search-form" method="get">
    <div class="request-verification">
        <?= $this->Element('specialist_search') ?>
...
<div class="inner">
    <div class="content-with-aside page-of-search">
        <div class="content">
            <div class="top-block">
                <div class="left">
                    <span class="filter-order <?= $this->request->query('order') === 'rate' ? 'active-filter-order' :
                        '' ?>">
            ...
            <div id="search-results">
                <?php foreach ($specialistList as $key => $specialist) { ?>
                    <?php if ($key === 3) {
                        $bannerCode = 'crrnbncnqtdpvvotwvwlyrjlcjkygcjfji';
                        ?>
                    ...
                    <?= $this->Element('card_specialist', ['specialist' => $specialist]) ?>
                <?php } ?>
                <?php if (empty($specialistList)) { ?>
                    <p>По выбранному запросу, пока нет специалистов нужного профиля.</p>
                    <p>
                        Однако, если вы знаете настоящих профи своего дела, напишите нам на почту topestet@ya.ru и мы
                        добавим их
                        на наш портал
                    </p>
                <?php } ?>
        ...
        switch ($currentSpecializationAlias) {
            case 'plastica':
                $bannerCode = 'bosgvefhtqkvkardhnpeiiccbybzbwwoih';
                break;
            case 'cosmetology';
                $bannerCode = 'mfyrkxyyvcpaknbajapcxmeyrdsuhnppat';
                break;
            default:
                $bannerCode = 'mudiitpnwcvvodkebjldrbwblmeddotqyw';
        }
