<?class AppModel extends Model {
	...
	public function beforeFind($queryData){
		...
		if (isset($this->virtualFields['is_top'])){
			$this->virtualFields['is_top'] = str_replace('%top_response_count%', $this->configs['top_response_count'], $this->virtualFields['is_top']);
    		$this->virtualFields['is_top'] = str_replace('%top_exnote_count%', $this->configs['top_exnote_count'], $this->virtualFields['is_top']);
		}
		if (isset($this->Specialist)){
			$this->Specialist->virtualFields['is_top'] = str_replace('%top_response_count%', $this->configs['top_response_count'], $this->Specialist->virtualFields['is_top']);
    		$this->Specialist->virtualFields['is_top'] = str_replace('%top_exnote_count%', $this->configs['top_exnote_count'], $this->Specialist->virtualFields['is_top']);
		}
		if (isset($this->Clinic)){
			$this->Clinic->virtualFields['is_top'] = str_replace('%top_response_count%', $this->configs['top_response_count'], $this->Clinic->virtualFields['is_top']);
    		$this->Clinic->virtualFields['is_top'] = str_replace('%top_exnote_count%', $this->configs['top_exnote_count'], $this->Clinic->virtualFields['is_top']);
		}
		if (isset($this->Sender)){
			$this->Sender->virtualFields['is_top'] = str_replace('%top_response_count%', $this->configs['top_response_count'], $this->Sender->virtualFields['is_top']);
    		$this->Sender->virtualFields['is_top'] = str_replace('%top_exnote_count%', $this->configs['top_exnote_count'], $this->Sender->virtualFields['is_top']);
		}
		if (isset($this->Parent) && isset($this->Parent->virtualFields['is_top'])){
			$this->Parent->virtualFields['is_top'] = str_replace('%top_response_count%', $this->configs['top_response_count'], $this->Parent->virtualFields['is_top']);
    		$this->Parent->virtualFields['is_top'] = str_replace('%top_exnote_count%', $this->configs['top_exnote_count'], $this->Parent->virtualFields['is_top']);
		}
	}
				...
				if (isset($source_data[$this->name][$key]) && !empty($source_data[$this->name][$key]['tmp_name'])){
					...
					foreach ($img_data as $prefix => $info){
						...
						if (isset($info['width']) || isset($info['height'])){
							...
							imagecopyresampled($result, $img, 0, 0, $x_source, $y_source, $width, $height, $width_source, $height_source);
	...
	function beforeSave(){
		...
		if (isset($this->_schema['ip'])){
			$this->data[$this->name]['ip'] = $_SERVER['HTTP_X_REAL_IP'];
		}
		return true;
	}
	
	function save($data, $validate = true, $fildList = array()){
