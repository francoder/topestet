...
class PaginatorHelper extends AppHelper
{
    ...
    public function sortDir($model = null, $options = [])
    {
        ...
        } else if (isset($options['order']) && is_array($options['order'])) {
            $dir = strtolower(current($options['order']));
        } else if (isset($params['order']) && is_array($params['order'])) {
    ...
    public function first($first = '<< first', $options = [])
    {
        ...
        if ((is_int($first) || ctype_digit($first)) && $params['page'] >= $first) {
            ...
            $out .= $after;
        } else if ($params['page'] > 1 && is_string($first)) {
            $options += ['rel' => 'first'];
    ...
    public function last($last = 'last >>', $options = [])
    {
        ...
        if ((is_int($last) || ctype_digit($last)) && $params['page'] <= $lower) {
            ...
            $out = $before . $out;
        } else if ($params['page'] < $params['pageCount'] && is_string($last)) {
            $options += ['rel' => 'last'];
            $out = $before . $this->Html->tag(
                    $tag, $this->link($last, ['page' => $params['pageCount']], $options), compact('class')
                );
        }
        return $out;
    }
    ...
    public function meta($options = [])
    {
        ...
        $params = $this->params($model);
        $urlOptions = isset($this->options['url']) ? $this->options['url'] : [];
        $links = [];
        if ($this->hasPrev()) {
     ...
     * @return string
     */
    protected function _pagingLink($which, $title = null, $options = [], $disabledTitle = null, $disabledOptions = [])
    {
        $check = 'has' . $which;
        $_defaults = [
        ...
        } else if (!$this->{$check}($options['model'])) {
            return '';
        }
