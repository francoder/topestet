...
class AuthComponent extends Component
{
    ...
    public static function user($key = null)
    {
        if (!empty(static::$_user)) {
            $user = static::$_user;
        } else if (static::$sessionKey && CakeSession::check(static::$sessionKey)) {
    ...
    public function redirectUrl($url = null)
    {
        ...
        } else if ($this->Session->check('Auth.redirect')) {
    ...
    protected function _unauthenticated(Controller $controller)
    {
        ...
        if (!$controller->request->is('ajax') && !$controller->request->is('json')) {
            ...
            $controller->redirect('/user/login/?redirect=' . $this->request->url);
            return false;
        }
