...
function array_search_sub($needle, $haystack)
{
    $key = key($needle);
    if (!is_array($haystack)) return false;
    $key_found = array_search($needle[$key], $haystack);
    if (!$key_found || $key != $key_found) {
        foreach ($haystack as $subarray) {
            if (array_search_sub($needle, $subarray)) {
                return true;
            }
        }
        return false;
...
function cut_by_words($text, $words_count = 50)
{
    $words = explode(" ", $text);
    if (count($words) > $words_count) {
        $words = array_splice($words, 0, $words_count);
        $text = implode(" ", $words) . "...";
    }
    return $text;
}
